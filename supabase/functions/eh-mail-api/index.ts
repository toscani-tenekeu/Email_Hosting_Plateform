...
async function ownedMailbox(mailboxId: string, userId: string, isAdmin: boolean) { let query = admin.from("eh_mailboxes").select("*, eh_services!inner(user_id)").eq("id", mailboxId); if (!isAdmin) query = query.eq("eh_services.user_id", userId); const { data, error } = await query.single(); if (error || !data) throw new Error("Mailbox not found."); return data; }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === "change_mailbox_password") { const mailbox = await ownedMailbox(String(body.mailboxId ?? ""), user.id, isAdmin); const password = String(body.password ?? ""); if (password.length < 10) throw new Error("The password must contain at least 10 characters."); if (password.length > 128) throw new Error("The password must contain no more than 128 characters."); await mailu(`/user/${encodeURIComponent(String(mailbox.email))}`, { method: "PATCH", body: JSON.stringify({ raw_password: password, change_pw_next_login: false }) }); const { error: auditError } = await admin.from("eh_audit_logs").insert({ actor_user_id: user.id, target_user_id: mailbox.user_id, action: "mailbox_password_changed", entity_type: "mailbox", entity_id: mailbox.id, ip_address: (req.headers.get("x-forwarded-for") || "").split(",")[0].trim() || null, user_agent: req.headers.get("user-agent") || null, metadata: { email: mailbox.email } }); if (auditError) console.warn("Mailbox password audit failed", cleanError(auditError)); return response({ ok: true, email: mailbox.email }); }
    if (action === "delete_mailbox") { const mailbox = await ownedMailbox(String(body.mailboxId ?? ""), user.id, isAdmin); await mailu(`/user/${encodeURIComponent(mailbox.email)}`, { method: "DELETE" }); const deletion = await admin.from("eh_mailboxes").delete().eq("id", mailbox.id); if (deletion.error) throw deletion.error; return response({ ok: true }); }
