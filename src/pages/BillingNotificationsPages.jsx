...
function BillingPage() {
  ...
  useEffect(() => { if (supabase && user) supabase.from('eh_wallet_transactions').select('*').order('created_at', { ascending: false }).limit(30).then(({ data }) => setTransactions(data || [])) }, [user?.id])
...
function NotificationsPage() {
  const data = useDashboardData()
  async function markRead(id) { if (!supabase) return; await supabase.from('eh_notifications').update({ read_at: new Date().toISOString() }).eq('id', id); await data.reload() }
