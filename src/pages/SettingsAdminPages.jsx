...
function SettingsPage() {
  ...
  async function save(event) { event.preventDefault(); const { error } = await supabase.rpc('eh_update_profile', { p_full_name: form.fullName, p_company_name: form.companyName }); setMessage(error ? error.message : 'Profile updated.'); if (!error) refreshProfile() }
...
function AdminPage() {
  ...
  useEffect(() => { if (!supabase) return; Promise.all([supabase.from('eh_profiles').select('*', { count: 'exact', head: true }), supabase.from('eh_services').select('*', { count: 'exact', head: true }), supabase.from('eh_provisioning_jobs').select('*', { count: 'exact', head: true }).in('status', ['pending', 'failed'])]).then(([u, s, j]) => setStats({ users: u.count || 0, services: s.count || 0, jobs: j.count || 0 })) }, [])
  async function credit(event) { event.preventDefault(); setMessage(''); const { data, error } = await supabase.rpc('eh_admin_credit_wallet', { p_customer_email: form.email, p_amount: Number(form.amount), p_reason: form.reason, p_idempotency_key: crypto.randomUUID() }); setMessage(error ? error.message : `Credited successfully. New balance: ${formatMoney(data.balance)}`); if (!error) setForm({ email: '', amount: '', reason: '' }) }
