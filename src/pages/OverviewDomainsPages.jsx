...
function DomainsPage() {
  ...
  return <DashboardLayout><PageTitle title="Domains" text="Email hosting services connected to customer-owned domains." action={<Link className="button" to="/checkout">Add domain</Link>} /><Notice><b>Domain registration is not provided here.</b> Every listed domain must remain registered and controlled by you.</Notice><section className="panel table-panel">{data.services.length ? <div className="responsive-table"><table><thead><tr><th>Domain</th><th>Plan</th><th>Mailboxes</th><th>Renews</th><th>Status</th><th></th></tr></thead><tbody>{data.services.map((service) => <tr key={service.id}><td><b>{service.domain_name}</b><small>{service.mailu_domain_created ? 'Provisioned in Mailu' : 'Waiting for Mailu provisioning'}</small></td><td>{service.eh_plans?.name || service.plan_id}</td><td>{data.mailboxes.filter((m) => m.service_id === service.id).length} / {service.mailbox_limit ?? 'Unlimited'}</td><td>{new Date(service.renews_at).toLocaleDateString()}</td><td><StatusBadge value={service.status} /></td><td>{!service.mailu_domain_created && <button className="button tiny secondary" onClick={() => retry(service.id)}>Retry</button>}</td></tr>)}</tbody></table></div> : <EmptyState icon={Globe2} title="No domains" text="Order email hosting for a domain you already own." action={<Link className="button" to="/checkout">Order service</Link>} />}</section></DashboardLayout>
}
